...
class CandidateModelStructureTests(unittest.TestCase):
    ...
    def test_profile_fit_review_freezes_current_profile_without_assigning_role(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temporary:
            ...
            self.assertEqual(history_window["window_start_year"], 2023)
            self.assertEqual(history_window["window_end_year"], 2026)
            self.assertEqual(history_window["required_year_count"], 4)
