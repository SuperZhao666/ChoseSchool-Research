...
生成时验证层级、必填标题和键的唯一性，非法嵌套直接失败。默认学校页只显示学院入口；全文搜索和旧锚点自动打开证据所属学院／项目／方向。展开与收起只作用于所选项目或共同说明；打印只包含所选项目的完整资料，并恢复打印前折叠状态。原始正文、表格和来源链接完整保存在同一HTML及根README，未采用实体标记的学校按正文标题与折叠资料生成本校内容导航，不推断新的招生实体。
...
项目内的“本项目详细内容”由实际标题锚点生成，方向选中时列出该方向与项目共同小节，点击直接跳到原正文；这不是新的招生项目或学校级分类。生成导航标记`data-reader-ui`并排除于研究全文检索和内容保真比较；正文、表格及来源始终属于原学院、项目和方向。湖南大学新增实质细节及纠错过程记录于`research/coverage/2026-09-14-hnu-detail-restoration.json`，不增加公开专题文档。
