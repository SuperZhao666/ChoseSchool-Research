...
决策模型只允许把前两种状态用作分科历史上下文；`secondary_mirror_final_crossmatch` 与 `dual_secondary_mirror_final_crossmatch` 只能在报告中显式标为旁证，不得进入择校排序、目标分、录取概率或“冲稳保”判断。双镜像状态低于“正式最终表 + 单份镜像”状态，不能因为逐人零冲突就抹掉来源等级。正式状态还必须同时保留 `population_scope`、专项/联培/委托排除规则、学习方式、方向边界和当年四科合同。相同总分不意味着相同科目结构；`917/961/902/861/840/839` 与 `408` 均为比较断点。读取时优先看中位数与 Q25—Q75，最低值、最高值和单年小样本不得建立“安全线”。
