...
| `panel_count` | 原文按学校锚点和顶层章节分成的阅读区数；默认仅显示首个学校区，其余正文完整保留 |
| `late_switch_school_count` | 有显式下半年改408公告标记的学校条目数；不代表同校所有项目或2027正式四科已核 |
...
阅读区 `data-panel` 使用既有 `school-*` 或章节标识；全部源区拼回必须逐字符等于README。旧锚点、搜索结果及前进后退均打开目标所属阅读区；本页展开、收起和打印不作用于隐藏学校。

学校正文中的 `switch-flag` 是呈现标记，不是数据库招生状态。仅 `data-status="late-announcement"` 触发红色侧栏；`data-announced` 必须为ISO日期，处于 `data-target-year` 前一年7月1日至12月31日。红标旁必须写明适用学院、项目和来源，不能按关键词生成。`switch-note` 的 `date-unresolved` 为滚动公告目标行首发日期未闭合，显示待核色；新增项目和历史改考只用说明文字，不继承红标。公告日期、旧新科目、同年四科目录资格及低分预测分别判断。

<!-- TraceId: 4633df94-71b7-4339-ae57-1ad2dd0576cb -->
成绩表的“当年初试科目与证据”紧跟年份，顺序为政治、外语、数学、专业课；按同项目同年度来源逐行标明科目及证据限制。它不改变原始分数、人数、项目或人口口径，不升级数据库招生状态。已有完整科目列无需重复。数值保真检查仅排除这个新增说明列后对照原始数值向量，仍逐项保护所有原成绩单元格；网页全文、表格和来源链接继续与README一致。混合试卷、多个年份、二手目录、仅成绩栏明示和缺失科目不得被自动填成统一408。

<!-- TraceId: d36580a9-e711-47ec-9786-53ec83803d21 -->
“成绩项目／初试字段”纵向分布表中的总分行也必须在表内附招生年份和四科，不能只依赖表前标题。历史回测与规则比较分别写输入年份和目标年份，换卷观察不得只写目标年的408。目录仅证明接收项目的考试要求，混有调剂或推免的统计另保留原考生试卷／人口未核限制。补证更新当前说明列，不修改归档原行、数据库状态或原始数值。

网页只对第2列表头恰为“当年初试科目与证据”的表增加 `exam-subjects` 样式类，使该列保留足够阅读宽度；窄屏仍在原横向滚动区域内查看，不截断科目、不移动或隐藏原始分数列。

<!-- 需求与结果复核约定 TraceId: 72cdaab7-d6a0-446e-9c80-472c3ea09cd0 -->
新增学校结果表沿用上述年度科目列，并分别保留复试总分线、拟录取初试最低分、分布中位数及招生计划阶段。“全日制、初试非零、备注空白”是明确的代理人口筛选条件，不自动等于已确认的普通一志愿；有一志愿原名单逐行交叉时另行说明。院系合并前后的同码项目不得默认视为稳定样本，先期改科预告必须与后续修订和最终目录一起解释。

本批内部底稿 `research/coverage/2026-09-13-demand-result-refresh.json` 仅保存来源定位、校验结果和匿名聚合，不增加数据库字段或招生状态；不保存姓名、考生编号和身份关联明细。P25/P75按 `(n-1)p` 线性插值计算；报名人数除以春季阶段计划只是描述比值，不是实际报录比或个人录取概率。完整择校解释继续只维护根README。

<!-- TraceId: 82224a8c-63f2-4d39-a923-a9069e5d11ac -->
学校正文标题后以 `school-tier` 段落明确标注院校层次，取值为 `985`、`211（非985）` 或 `双非`；这里双非指非985、非211，不等于非双一流。标签依据教育部历史名单，描述学校而非专业强弱或录取难度。网页导航从同一段落读取标签，不另维护学校分类表；标签缺失时生成失败，不把未核学校默认判为双非。
...
<!-- 合工大 2023 双镜像交叉状态续补 TraceId: 5dc980b1-aa27-4618-b214-6119e3632826 -->
<!-- 中国海洋大学 2025 公开逐行镜像观察状态续补 TraceId: 635ff73b-336e-4698-b78b-dc3c40eb4b54 -->
...
| `secondary_blurred_final_order_subject_set_identification` | 校方复试表完整给出稳定编号与四科，但最终人口只能从第三方完整截图观察，且编号／姓名已脱敏，只剩专业内顺序、姓氏首字和专项标记可见。只有在另一专业的独立对照序列可验证同一排序规则、目标序列在强制专项边界后可被完整枚举时，才允许发布所有相容集合的逐科统计严格最小—最大区间；必须同时保留最终人数来源冲突。禁止复原或发布个人身份、禁止任选一种映射形成点估计，禁止算作官方交叉或进入模型 |
| `secondary_aggregate_constrained_final_subject_set_identification` | 校方完整复试表逐行给出四科，校方规则又能确认目标最终人口只能来自该复试池，但正式最终名单缺失；第三方仅给最终 `n`、总分最小值、最大值和取整均值。只有在取整规则能由相邻年度“第三方整数值—校方精确值”独立校准、全部约束可同时满足且所有相容子集都已穷举时，才允许发布每个统计量在所有相容子集上的严格最小—最大区间。第三方聚合任一字段冲突、取整规则未校准或最终人口可能包含池外调剂时，均不得使用。禁止任选一个相容子集形成点估计，禁止算作正式最终人口或进入模型 |
| `secondary_visible_mirror_final_subject_observation` | 单份公开可见的第三方逐行表同时给出四科、人口备注和录取状态，且逐行四科加总无误；但没有取得校方最终附件，无法与正式最终人口按稳定编号交叉，镜像内部人数冲突也必须保留；只允许作为最低一级的人类可读匿名观察，不算官方精确格 |
...
## 2025—2026 改考案例面板 CSV（研究附件，不是数据库迁移）
...
| 字段 | 类型／单位 | 含义与限制 |
|---|---|---|
| `trace_id` | 文本 | 原始聚合核验的TraceId，发布副本保持原值 |
| `id` | 文本 | 此研究表内唯一项目配对标识，不替代数据库项目ID |
| `school` | 文本 | 学校名称；学校等权时的分组单位 |
| `group` | 文本 | `first_408_2026`为2026首次考408；`continuous_408`仅表示2025和2026都考408，不代表长期稳定未受调整 |
| `tier` | 文本 | 分析纳入层级：A固定筛选代理；H前身／外语扩展；B_catalog科目目录镜像旁证；C_stage阶段放宽；C_population人口放宽；X_direction方向不清、排除全部汇总。不是官方证据等级或招生状态 |
| `n25`, `n26` | 整数／人 | 对应筛选人口的公示记录数，不替代普通统考名额、报考人数或实考人数 |
| `median25`, `median26` | 数值／初试总分 | 对应筛选人口的初试中位数；不是复试线或总评成绩 |
| `mean25`, `mean26` | 数值／初试总分 | 原审计表发布的初试均值，通常已舍入到两位小数 |
| `delta_median`, `delta_mean` | 数值／分 | 2026对应值减2025对应值；均值差由已舍入均值计算，不冒称无限精度 |
| `before_project`, `after_project` | 文本 | 两年的学院／专业／方向身份，保留拆码和前身映射 |
| `course25`, `course26` | 文本 | 两年科目合同原值；选考外语保留菜单，不能当所有人都考英语二 |
| `population25`, `population26` | 文本 | 各年的实际筛选条件；保留备注空白、专项缺列、一志愿、学院累计等差异 |
| `flags` | 文本 | 来源、阶段、人口和近期改考等限制；不能忽略后再把记录全部混算 |
| `file` | 相对仓库路径 | 承接数值的既有专题文档，不是新原始来源行 |
| `line25`, `line26` | 正整数／行 | 2026-09-05核验时两年原聚合行的定位；后续文档编辑可能使行号移动，应同时核年份和项目 |
| `url25`, `url26` | URL | 原件或官方发布锚点；存在链接不等于本轮重新下载成功，实际载体见flags和专题 |

项目等权为各项目差值的算术平均；学校等权先对同校项目差值平均、再对学校平均。不得使用中位数相加计算四科总分，不得将公示人数当权重后冒称总体中位数。缺失值留空，不填0；本表有中心值也仍可能因层级X被排除。CSV新增列只用于这份研究附件，没有新增应用公共命令、数据库字段或招生状态。

2026-09-05来源说明纠正TraceId：`12b10116-09ad-4a8a-b786-73ec9056d13d`。郑大2025行的数值不变，`population25`按校级最终原件改为全日制非定向专项无96人，剔除定向少民骨干1人；`url25`改为校级最终公示，`flags`记录已登记留存原件、当前验证码限制和纠正TraceId。原始`trace_id`保持；更正前研究副本保存在本机审计目录，不覆盖原始来源行或数据库。
...
| 字段 | 定义与限制 |
|---|---|
| `trace_id` | 原始取证和计算TraceId；发布副本保持原值 |
| `task_id` | 项目与目标年组成的任务标识；每个标识固定三条规则 |
| `cohort` | `compatible_subject_proxy`科目相容代理层；`broad_professional_sensitivity`专业宽口径；`stable_non408_separate`连续自命题单列；`paper_switch_stress`目标年换卷观察。不得混层报告主结果 |
| `school` | ECNU华东师范、XJU新疆大学、ZZU郑州大学、SWJTU西南交通、BJTU北京交通；学校等权用此字段分组 |
| `project` | 当年项目标识，不能省去学院和方向差异 |
| `target_year` | 被检验结果所属招生年，不是公告年 |
| `simulated_origin` | 模拟起点，固定为目标招生年之前一年的11月1日；仅作事后时间边界 |
| `design` | 标明规则和样本在本轮事后选择、没有前瞻注册，不得删去后当真实历史预测 |
| `target_statistic` | 固定为限定人口拟录取者的初试总分中位数，不是复试线或个人成绩 |
| `rule` | L1=上年中位数；A2=前两年中位数的算术平均；H2=前两年中位数中较高值 |
| `prev2_year`, `prev1_year` | 输入的前两年与上一年；缺乏相容历史窗口时相应值留空 |
| `prev2_median`, `prev1_median` | 对应限定人口初试总分中位数，单位分；不是所有报考者分布 |
| `prev2_n`, `prev1_n` | 对应输入公示筛选人数，不等于统考计划 |
| `actual_median`, `actual_n` | 目标年公示人口的实际中位数和人数；只用于核对误差，不参与该任务的预测计算 |
| `prediction` | 依相应规则算出的历史模拟预测值；不可算则留空 |
| `error_pred_minus_actual` | 预测值减实测值，单位分；负数为低估 |
| `absolute_error` | 误差绝对值，单位分；有效行可用于同口径MAE，不是置信区间 |
| `error_direction` | under低估、over高估、equal相等；不可算时留空 |
| `eligibility` | `available_in_stated_layer`只在指定层可算；`unavailable`无可用窗口。不是招生状态 |
| `missing_reason` | 不可算的具体原因。不可算行的预测和误差都为空，不填0 |
| `courses` | 历史至目标年的科目说明；换卷必须显式保留 |
| `population_boundary` | 备注代理、专项清洗、培养方向变化等限制；不能忽略后称纯普通同路线 |
| `source_file`, `source_line_refs` | 承接原统计的仓库文档和本轮核验行号；后续编辑可能移动，应同时核项目、年份和源链接 |
| `prev2_source_url`, `prev1_source_url`, `target_source_url` | 历史输入与目标结果的原件或发布入口；缺失链接不猜补，不代表数值自动无效或来源已直取 |
| `source_correction` | 已发现的来源归属或专项标签纠正，不覆盖旧原始主张 |
| `source_timing_note` | 日期核验和本轮访问情况的限制；路径日期不能不加区分地冒称当前公示页日期 |

MAE先在相同层、相同有效任务中计算。学校等权先计算每所学校的任务MAE再平均；方法比较只用各规则共同可算的任务。缺失历史不补零；同校多年度并不独立；事后模拟起点不等于已恢复所有历史网页快照。CSV只是文档附件，没有增加应用命令、数据库字段或招生状态。

## 西农2026复试名单与拟录取选择聚合CSV
...
| 字段 | 定义与限制 |
|---|---|
| `trace_id` | 原始匿名聚合计算TraceId；发布副本保持原值 |
| `admission_year`, `college_code`, `program_code`, `program_name` | 招生年、学院及专业身份；代码保留前导零，不新增方向映射 |
| `record_type` | cohort_status为名单状态计数；admitted_composition为拟录取者低分构成；initial_rank_displacement为已知结果子群的初试排序与结果比较。不是数据库招生状态 |
| `population_scope` | 复试名单精确专业名，排除明确少骨和另列非全日制。无独立非定向/方向字段，不推断其他属性 |
| `score_band` | ALL总体；五分箱为<300、300-319、320-339、340-359、>=360；低分构成另用<300、<320，后者包含前者。均按初试总分，排名行留空 |
| `N_listed` | 列入复试名单的普通代理人数，不是实际参加人数 |
| `K_observed` | 本份结果表回连到的人数。排名行也用这个子群；不把未知者纳入排名 |
| `A_provisional` | 备注精确“拟录取”的人数，不含拟递补，不代表实际入学 |
| `B_reserve` | 备注“拟递补”的人数，保持独立 |
| `R_interview_fail` | 备注“面试不合格，不予录取”的人数，保留原因，不改写成初试低分淘汰 |
| `U_unmatched` | 复试名单内但本份结果表未匹配的人数；未知不等于缺考、放弃或不予录取 |
| `A_over_N_listed`, `A_over_K_observed` | A/N、A/K，0至1的小数；分别描述当前已证拟录取占原名单比例、已见结果子群构成，不是完整最终录取率或2027概率 |
| `metric` | fraction_of_observed_provisional_admits_below_score为低分构成；top_n_not_admitted为初试前n中未写拟录取；beyond_n_admitted为前n外写拟录取。后两项不是独立证据 |
| `numerator`, `denominator` | 只供低分构成行使用；分子为小于阈值且拟录取的人数，分母为该项目公示精确拟录取人数。不能倒读成低分考生的录取率 |
| `count_lower`, `count_upper` | 排名计数的最小/最大值，保留跨边界同分排列的不确定性；不是置信区间 |
| `rank_threshold_n` | 以当次拟录取人数定义的排序边界n，不是提前下达的计划名额 |
| `initial_score_cutoff`, `tie_positions` | 已知结果子群初试降序第n位的分数及同分位置范围；不能把原PDF行序当官方初试同分排序规则 |
| `retest_pages`, `result_pages` | 两份原PDF的物理页码，从1起；范围表示项目记录跨页 |
| `retest_source_url`, `result_source_url` | 原校方PDF地址；本轮使用已保留原件重新核验，不声称每次在线附件均可读 |
...
`research/coverage/2026-09-13-spring-plan-originals.json` 记录本次六校的来源、招生年、项目、学习方式、普通／专项口径、阶段人数、匿名分布和已纠正的旧推导。TraceId：`a5c86a8e-e81a-4119-bff8-a45ec8b5de6b`。它是内部取证底稿；唯一完整阅读正文继续维护在根 README 各校原位置，没有新增数据库字段、命令或招生状态。

`spring_plan`、`public_exam` 均指原文指定阶段的计划，不是最终入学人数，且必须连同联培及培养专项备注解读。`policy_retest_line` 未取得时为空，名单观察最低不替代该值。`final_population.verified_n` 为空表示最终净人数未知；累计曾获拟录取公示人数不能代替最终净人数。`initial_publication` 与 `supplement` 分别保留首榜和递补，旧“恰退出一人”的分布区间仅作条件推导留存。二等功推免与退役大学生专项分列，空白不升级为官方零；四科必须与同年同项目正式目录绑定。2025 报考统计不与 2026 拟录取数混算。浏览器实读记录与可下载原始 PDF 的哈希明确区分，未下载的原件不伪造指纹。公开底稿仅含聚合和材料元数据，逐人记录及身份关联留在库外。

<!-- TraceId: 08b14bba-791f-4838-89fc-df06a775482c -->
历年分数表的展示科目按同一学校、项目、招生年和统计人口绑定，禁止按表格出现顺序循环分配年份。横向分科列与纵向成绩字段均须保留同行完整科目及证据状态；未知、镜像、预告和正式目录不得互相替代。内部审计 `research/coverage/2026-09-13-score-year-context-corrections.json` 记录原行定位、年份更正、增列数量与来源快照；该审计不改变原成绩及人口列。中传2026的初试门槛、初试与材料合成的复试准入综合门槛、最终录取总成绩为三种独立字段，不能共用“分数线”含义。

<!-- TraceId: b620e1e2-a0b0-4627-a110-a9455976871d -->
`research/coverage/2026-09-13-ai-demand-and-current-notices.json` 为本轮AI需求与公告复核的内部底稿。克拉玛依的 `matched_final_to_first_choice` 表示校级拟录取记录与同年一志愿名册的完整编号匹配数；`initial_score_fields_verified` 是政治、外语、两门业务课及总分共五个核对字段，不是五门考试。方向子集仅继承原报考标签，校级拟录取表没有最终培养方向；`ordinary_proxy_definition` 保留全日制、原名册非定向、备注空白等条件，不升级成专项已全部排除。`q25_linear`／`q75_linear` 为线性插值分位数，少于3人的方向不发布成绩分布。推免拟计划与目录余额不填普通净名额；未取得的报考统计保持空值。此底稿不增加数据库命令、字段或招生状态，公开研究全文仍只有根README各校原位置。


<!-- TraceId: 9ccf3fb0-5aa8-465b-968a-3dac2299bb85 -->
`research/coverage/2026-09-14-bit-885-408-and-programs.json` 是北京理工大学本次定向取证的内部聚合底稿，完整阅读正文仍在根README学校069。它记录2026全校27单位、351方向的扫描范围，17个408方向、其中10个数学二方向，原目录单元格及去重统考计划120；2027软件工程的科目调整公告独立于尚未取得的2027完整目录。各年科目、学院、方向、阶段和培养地点不得相互继承。

`current_selection` 只描述当前数学二＋408、允许英语一及交叉专业的筛选偏好，不改变数据库中同年同项目101＋204＋302＋408正式确认规则。`cs_retest_aggregate`、`zh_retest_aggregate` 是进入复试名单的匿名分布，不是报名、实到或录取分布；`cs_2024_final_aggregate` 仅为对应两批拟录取公示。`ai_medical_aggregate` 保留AI大专业混合810与408的不可拆分边界，以及医学表中专项／方式未披露的限制。分位数使用(n-1)p线性插值；未知人口属性不补成普通、未知数据不填0。新增比对不输出个人标识或逐人关联记录。

885旧题为不完整回忆版，408资料分别标扫描、重排或存在差错的再现，题目、题号和分值缺失不能用解题者补写冒充原卷。数字表演2025/2026业务二均分之差是不同入围群体的描述性差异，不能作为软件工程2027的因果降分量；复试线、复试名单最低与拟录取最低是独立概念。此文件不增加数据库公共命令、字段或招生状态。


<!-- TraceId: c3810655-9d25-4f82-a7e0-4a4cb679d9ee -->
`research/coverage/2026-09-14-bit-experience-credibility.json` 是北理考生亲历与平台线索的内部来源审计；公开完整正文仍在根README学校069。`posted`记录发布日，`cohort`记录作者实际申请／入学届次，两者不互相代替；`type`为来源性质而非数据库招生状态。`read_scope`区分全文、公开片段和受限内容；`match`、`limits`与`credibility`保留项目、阶段、作者利益和独立验证边界。`retrieval`仅对实际取回的响应记状态与哈希，失败响应和web实读不伪造原件指纹。`usable`记录允许支持的具体描述，不把本人感受改成统一规则。相同作者续帖和转载链去重；推免／统考、数一／数二、在读／入学前、学院／精确专业方向分开，待核平台数字不覆盖正式统计。所有作者成绩仍为自报，未作个体名册关联；公开不含联系方式及原文全文。此审计不新增应用命令、数据库字段或官方确认状态。

<!-- TraceId: 4fa2880a-e3d3-40b3-a2d9-8c69ad9fd505 -->
根README以“学校 → 学院 → 具体招生项目 → 研究方向”组织实体。学校029（湖南大学）与069（北京理工大学）的科目、历年成绩、名额、复试和培养信息均属于对应项目；不再提供学校级的八类资料切换。标记为阅读结构，不新增数据库招生状态，也不把历史项目或公告待目录项目确认为当年招生。

| 阅读字段／标记 | 含义与边界 |
| --- | --- |
| `section.school-college`、`data-college-key`、`data-college-title` | 学校内招生学院／单位。每个学院至少有一个项目；跨学院同名专业分开。 |
| `section.admission-project`、`data-project-key`、`data-project-title` | 只能属于一个学院。项目键在全校唯一，例如 `207-085404` 与 `241-085404`；不是仅按六位专业代码合并。 |
| `data-project-status` | 项目当前证据与报考范围的可见说明，例如2027改考公告、2026目录记录或历史数一对照。它不替代正文中的年度、学习方式、专项和正式目录证据。学校存在已核下半年公告且项目状态明确含“改考408”时，该项目入口标红。 |
| `section.research-direction`、`data-direction-key`、`data-direction-title` | 只能属于一个具体项目，方向键在项目内唯一。点击方向定位方向说明，项目共有科目、成绩及复试资料继续显示；共有名额不复制成方向独立名额。 |
| `section.admission-notes`、`data-notes-title` | 学校级共同口径与来源核验，只能位于学院之外，不作为招生项目或研究方向。 |
| `data-active-entity` | 当前浏览会话所选实体的锚点。不会写入招生事实；选择学院仅显示其项目入口，选择项目才显示该项目完整正文。 |
| `college-school-069-207`、`project-school-069-207-085405`、`direction-school-069-207-085405-00` | 从已有层级键生成的稳定分享锚点；分别定位学院、项目与方向。旧正文锚点继续有效。 |
| `data-reader-ui` | 从实体标题生成的导航、入口及面包屑，不属于研究正文，全文搜索与保真核对排除这些重复的界面文字。 |

生成时验证层级、必填标题和键的唯一性，非法嵌套直接失败。默认学校页按学院分组显示具体项目入口；全文搜索和旧锚点自动打开证据所属学院／项目／方向及阅读栏目。展开与收起只作用于所选项目或共同说明；打印只包含所选项目的完整资料，并恢复打印前折叠及栏目可见状态。原始正文、表格和来源链接完整保存在同一HTML及根README，未采用实体标记的学校按正文标题与折叠资料生成默认收起的本校内容导航，不推断新的招生实体。

`research/coverage/2026-09-14-bit-reading-layout.json`保留上一版资料分类调整的历史审计，不代表现行导航。所有分表仍须保留学院、项目、方向、年份、当年科目与统计人口边界；新增表头不意味着新增样本或证据。

<!-- TraceId: 6e1e24ca-cd0a-45ae-9808-f82bd5b15ad8 -->
湖南大学的`csee-085400`用于计算机学院合并项目，`former-csee-085400`用于原信息科学与工程学院2023—2025历史项目；阅读键不充当官方学院代码。`merged`说明合并招生，`computer-history`、`software-history`定位历史方向，均不虚构官方数字方向代码。2027推免名称不能升级为独立普通统考项目；2026年5月的改408公告属于上半年，不标红。内部归属及保真审计记录在`research/coverage/2026-09-14-hnu-entity-hierarchy.json`，完整研究正文仍在根README。

<!-- TraceId: c4d5b28b-0b67-4d2f-89e1-b46eb1090822 -->
全校统一阅读排版（TraceId: 12977bfb-1cef-4a83-b35a-0c58141c1a37）：学校正文的表格统一添加`readable-table`，五列及以上或任一正文单元格可见文本超过140字符时添加`record-table`，按原表的一行展示一张记录卡。超过80字符的单元格标记`long-field`并独占一行。已有`data-reading-layout="records"`继续兼容。生成的`data-label`逐列取自原表头；标准九列年度成绩表将普通录取人数、复试总分线、最低、中位、平均、最高分排成卡内指标，年度、当年科目和学院方向保留在同卡。原表只有一份数据，未知文本不换算成数字；其余小表宽屏保留紧凑表格，容器宽度不超过40rem时同样显示记录卡。跨校依据维持原排版。语义表头和列关联保留，打印同样保留每行完整内容。

项目内原有“本项目详细内容”锚点仍可解析；当前默认使用下述阅读栏目，避免标题长目录占据首屏。生成导航标记`data-reader-ui`并排除于研究全文检索和内容保真比较；正文、表格及来源始终属于原学院、项目和方向。湖南大学新增实质细节及纠错过程记录于`research/coverage/2026-09-14-hnu-detail-restoration.json`，不增加公开专题文档。

本校内容导航由非实体学校的实际h4—h6标题与summary生成，缺少锚点时使用稳定的`reading-school-NNN-序号`，只导航原正文；点击折叠资料会打开对应正文及祖先。导航使用`data-reader-ui`排除于全文索引，按钮可收起目录，打印不包含目录。

<!-- TraceId: 620a4022-cd30-48ab-992b-4a26e0cff841 -->
2026年9月17日增加13校19个来源归属明确的项目，合计15校34个项目使用实体阅读。按原资料明确的学院／代码分组，不能把同码跨学院材料合并；厦门的`software`为阅读键，不冒充正式学院代码。其他项目和归属未完成的学校材料继续保存在同校共同说明中。本次不是全校目录核验，没有新增招生确认状态。

| 阅读标记 | 当前行为与边界 |
| --- | --- |
| `.project-verdict` | 根README中的简短项目结论，保留年份、名单人口及待核限制；不从最低分生成个人安全线。 |
| `.school-background` | 浏览器将实体学校的开头说明折叠收起；完整原文保留，搜索可打开。 |
| `.reading-tabs`、`data-reading-topic` | 项目内根据现有小节标题生成招生、分数、复试、培养等栏目；复合小节可以在两个相关栏目出现，但只有一份正文节点。栏目归类是界面用途，不是官方招生分类。 |
| `.reading-chapter` | 原小节内容的可见性容器，不复制、重写或丢弃表格与段落；启用脚本前后，排除界面文字的正文逐字符相同。 |
| `view-project-…-scores`等 | 可分享的项目阅读栏目链接，刷新和前进后退可恢复栏目。旧章节链接继续自动切换至所属栏目。 |
| `.direction-choices`、`data-reading-direction` | 从已标记的真实／历史方向生成选择项；可单看所选方向和项目共同规则，历史方向不升级为当年独立招生项目。 |
| `完整资料`与打印 | 显示当前项目全部小节和方向；打印后恢复先前栏目、方向与折叠状态，其他学校仍隐藏。 |

`research/coverage/2026-09-17-project-reading-ownership.json`保存原资料归属、逐段保留审计和覆盖边界；不新增公开报告，也不要求读者离开README拼接正文。

<!-- TraceId: 8d98f22a-7a30-4d09-b60c-59c407ea8b93 -->
全库阅读覆盖扩展为104校：93校273个项目／培养路线／历史入口，11校明确展示证据缺口。此前15校34项目是前一阶段快照。根README仍是唯一公开正文；`all_school_reading_catalog.py`仅为一次性迁移底稿，运行时和构建均不读取它。后续内容更新直接写README，禁止维护第二份择校池。

| 阅读字段或行为 | 定义与限制 |
| --- | --- |
| `.project-source-selector` / `data-project-terms` | README内的已审核项目身份线索，供界面在同校原材料中定位；不是招生状态、正式学院代码或数据裁决。 |
| `.project-excerpts` | 选中项目后按需生成的展示副本，标记`data-reader-ui`避免重复检索；原文仍只有一个权威位置。完整原表行逐格复制，禁止剥离年份、科目、人口与备注。 |
| `.evidence-scope` | 提醒独立项目记录与共用／比较材料的区别；无法定位到单个项目的多项目材料进入来源栏，不能包装为本项目事实。 |
| `.evidence-context` | 每张表旁保留同段文字、统计限制和来源，可展开；不是删除或摘要替换。 |
| `.school-reading-status` | 精确项目缺失的学校展示已查范围与缺口，不创建假学院或假招生项目。 |
| 空栏目 | 显示尚未独立形成可归属资料；不借其他项目、其他年或其他学习方式补齐。 |
| 归属与年度 | 同码跨学院、历史改码、办学实体、方向与全日制／非全分别保留；阅读路由不代表独立分配名额或排名。 |

`research/coverage/all-school-reading-ownership.json`记录104校逐校覆盖与插入式保真检查。扩展没有修改原表或数据库状态，当前年下半年红标继续仅限已有正式公告明确覆盖的项目。


<!-- TraceId: 6cb4aa8d-3c3c-415f-a2c8-9d4c2dd9525f -->
2026年9月17日自命题续查：当前个人筛选为数学二必需、英语二优先且接受英语一，优先985的数据结构／算法自命题。此为阅读和个人决策条件，不新增数据库状态，也不改变strict_match与official_confirmed的22408证据合同。新增山东大学074-085412、077-085412、077-145200三个正式历史项目入口；同轮新增湖大cyber-085400历史项目与2027改考待核入口；当前104校、93校结构化、278个项目／培养路线／历史入口、11校明确缺口。上文273为扩展前快照。中央民族大学2027科目存在与项目映射分开存述，镜像大纲不升格官方附件。

<!-- 自命题续核 TraceId: 6cb4aa8d-3c3c-415f-a2c8-9d4c2dd9525f -->
9月17日续核增加华东师大162大数据的原生项目入口。其2025、2026统计是实考人数与非专项一志愿录取，不是报名数或个人录取率；同码829的年度范围变化随分数表保留。复试转述标为二手待核，未改变数据裁决状态。
