...
<!-- TraceId: 36aa879d-d3fd-4817-aae7-3552cf393aef; navigation: aab7da2b-f5ac-4368-932e-f8f414c5ad61 -->
根README可用`section.school-topic`、`data-topic-key`、`data-topic-title`标记同一学校的资料栏目；目前仅学校069使用八栏。栏目是完整正文的阅读分组，不是独立报告、招生项目确认或数据库状态。`data-active-topic`仅记录当前浏览会话的选中栏目，切换学校、全文搜索、旧锚点定位和打印必须保留正确可见范围；所有栏目正文始终保存在同一HTML及README内，打印临时展开本校全部栏目后恢复。导航按钮是由栏目标题生成的界面文字，不计入研究正文保真内容。`research/coverage/2026-09-14-bit-reading-layout.json`记录拆分前后哈希、八栏映射、项目分组和逐实质行／原来源链接保留审计；原29张表按项目拆成43张表，新增表头不意味着新增样本或证据。每份独立表仍须保留学院、专业、年份、当年科目和统计人口边界。
