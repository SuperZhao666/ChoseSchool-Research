...
生成时验证层级、必填标题和键的唯一性，非法嵌套直接失败。默认学校页只显示学院入口；全文搜索和旧锚点自动打开证据所属学院／项目／方向。展开与收起只作用于所选项目或共同说明；打印只包含所选项目的完整资料，并恢复打印前折叠状态。原始正文、表格和来源链接完整保存在同一HTML及根README，未采用实体标记的学校展示不变。
...
`admission-project`的`data-reading-layout="records"`是可选阅读布局：项目内五列及以上表格按原表的一行展示一张记录卡。生成的`data-label`逐列取自原表头；标准九列年度成绩表将普通录取人数、复试总分线、最低、中位、平均、最高分排成卡内指标，年度、当年科目和学院方向保留在同卡。原表只有一份数据，未知文本不换算成数字；未选择此布局的项目沿用原表格显示。语义表头和列关联保留，打印同样保留每行完整内容。
